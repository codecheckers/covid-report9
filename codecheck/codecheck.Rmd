...
were repeated using the public CovidSim implementation that was first
released in April 2020 onto Github rather than the private code used
to generate the initial findings.  Small variations (typically a few
percent) in the numbers were observed between Report 9 and my runs
which are due to several factors:
...
These jobs were then run, and on a high-performance workstation took
about 3 days, and about 1 day on a departmental HPC cluster.  To
repeat these runs on other computers will depend on your job
submission system.  However on a linux machine, one simple way to
start the jobs (with -j being the number the number of jobs to run in
parallel) is:
...
folder.  Two R scripts [xxx] provided by Prof Ferguson was used to summarise
these runs into two summary files: [GB_suppress_release/mean8/stats_contain.csv](<https://github.com/codecheckers/covid-report9/blob/master/GB_suppress_release/mean8/stats_contain.csv>)
and [GB_mitigation_release/MeanT8_NR10/stats_mitigation.csv](https://github.com/codecheckers/covid-report9/blob/master/GB_mitigation_release/MeanT8_NR10/stats_mitigation.csv).

These files were compared against the values generated by Prof
Ferguson and stored in the Excel spreadsheets using an R script.
...
these pivot tables to include for this report.  The excel summary
spreadsheets are available in the repository.
...
Please note that DOIs can take 1-2 days to register after the
